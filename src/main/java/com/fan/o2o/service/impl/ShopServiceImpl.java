...
import com.fan.o2o.dao.ShopDao;
...
public class ShopServiceImpl implements ShopService {
	...
	public ShopExecution addShop(Shop shop, ImageHolder thumbnail)
		...
		try {
			//给店铺信息赋初始值
			shop.setEnableStatus(0); //初始化为未审核
			...
			if (effectedNum <= 0) {
				/**
				 * 这里用ShopOperationException而不用Exception的原因
				 * 只有是ShopOperationException或者其子类才会进行回滚操作
				 * */
	...
	public ShopExecution modifyShop(Shop shop, ImageHolder thumbnail)
		...
		if (shop == null || shop.getShopId() == null) {
			...
			try {
				//判断是否需要处理图片，一旦上传新图片，就需要删除旧的图片
				if (thumbnail.getImage() != null && thumbnail.getImageName() != null && !"".equals(thumbnail.getImageName())) {
					Shop tempShop = shopDao.queryByShopId(shop.getShopId());
					if (tempShop.getShopImg() != null) {
						ImageUtil.deleteFileOrPath(tempShop.getShopImg());
					}
					addShopImg(shop, thumbnail); //shop的image已经被赋值
				}
				//更新店铺信息
				shop.setLastEditTime(new Date());
				...
				if (effectedNum <= 0) {
					...
					return new ShopExecution(ShopStateEnum.SUCCESS, shop); //状态信息和shop实例
				}
			}
			catch (Exception e) {
